...
int evtid_begin_split, evtid_end_split;
...
void mpi_mat_mat_mul(int m, int n, int k, double* A, double* B, double* C, MPI_Comm comm, int* p) {
    ...
    MPE_Log_event(evtid_begin_split, rank, (char*)0);
    ...
    MPE_Log_event(evtid_end_split, rank, (char*)0);

    MPE_Log_event(evtid_begin_matrix_mul, rank, (char*)0);
    mat_mat_mul(nn[0], nn[1], k, AA, BB, CC);
    MPE_Log_event(evtid_end_matrix_mul, rank, (char*)0);
...
int main(int argc, char** argv) {    
    ...
    evtid_begin_split = MPE_Log_get_event_number();
    evtid_end_split = MPE_Log_get_event_number();

    MPE_Describe_state(evtid_begin_matrix_mul, evtid_end_matrix_mul, "Matrix-Matrix Mul", "red");
    MPE_Describe_state(evtid_begin_split, evtid_end_split, "Split data between processes", "blue");
